REBOL [
	Date: 16-Aug-2005/8:51:29+2:00
	Title: "atcp:// protocol (Asynchronous TCP)"
	Purpose: {
		A protocol handler (atcp://) to do asynchronous I/O with TCP.
	}
	Author: ["Romano Paolo Tenca" "Ladislav Mecir"]
	Email: rotenca@tiscalinet.it
	File: %atcp-protocol.r
	License: {
Copyright (C) 2003-2004 Romano Paolo Tenca
...
http://www.gnu.org/copyleft/gpl.html
	...
	Version: 0.2.1
	Note: {BETA version, requires Rebol View 1.2.10 or same level Core}
	history: [
	...
	set 'async-root-protocol make root-protocol [
		...
		open-proto: func [
			...
			locals: port/locals: make either port/locals [port/locals][object!][
				...
				; handler added by Ladislav
				handler: none
				user-detect: any [
					all [
						; changed by Ladislav
						handler: select custom 'handler
						handler/port: port
						func [port event [error! word!]] [
							touched: now/precise
							either error? event [handler/error event] [
								either in handler event [
									either event = 'close [
										unless closing? [handler/close]
									] [handler/:event]
				...
				timeout: none
				timeout-awake: [
					if touched + timeout < now/precise [handler/timeout]
				]
			]
			either detect [
...
close-iatcp: func [port] [
	wait-stop port
	either 1 >= length? port/locals/events [
		port/locals/closing?: true
		close port
		port/locals/closing?: false
	] [
		print ["closing event:" mold port/locals/events]
		foreach event bind difference first atcp-handler-proto [
			self port
		] in port/locals/handler 'self [set event :close-iatcp]
	]
]

atcp-handler-proto: make object! [
	port: none
	error: func [error [error!]] [
		print ["error event:" mold disarm error now mold query port]
		close-iatcp port
	]
	dns-failure: does [
		print "DNS failure"
	...
	dns: does [
		print ["dns success:" port/sub-port/host ":" port/sub-port/port-id]
	]
	max-retry: does [
		print "cannot connect"
		close-iatcp port
	]
	connect: does [print "Connected"]
	read: does [print ["Data read:" copy port]]
	write: does [print "data have been written"]
	close: does [
		print ["peer closed the port, time:" now mold query port]
		close-iatcp port
	]
	timeout: does [print "timeout"]
	unknown: func [event [any-type!]] [
		print [
