...
;			print ["Start sending" file]
			...
			either dir? file: to-file file [
				insert/only files-to-transmit file: reduce [
					ansno dr: to-binary mold/only read join %chat-uploads/ file file select post-callbacks cid 0 0
				]
				...
				upload-dir: %chat-uploads/
				if %./ <> (first split-path file) [
					upload-dir: join %chat-uploads/ first split-path file
					file: second split-path file
				]
					
				insert/only files-to-transmit file: reduce [
					ansno file-port: open/binary/direct/read join upload-dir file file select post-callbacks cid 0 0
				]
